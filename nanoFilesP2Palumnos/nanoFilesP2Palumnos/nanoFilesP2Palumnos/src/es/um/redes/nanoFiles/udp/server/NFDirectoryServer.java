...
public class NFDirectoryServer {
	...
	// Vamos a crear un Array de FileInfo para guardar 
	// la información de los ficheros subidos al servidor.
	//LinkedList<FileInfo> listaFicheros = new LinkedList<FileInfo>();

	HashMap<String, FileInfo> ficheros = new HashMap<String, FileInfo>();
	...
	public DatagramPacket receiveDatagram() throws IOException {
		...
		while (!datagramReceived) {
			...
			// Creamos el datagrama utilizando el buffer.
			...
			// Recibimos el datagrama a partir del socket.
			socket.receive(datagramReceivedFromClient);
			
			// Podemos comentar esta parte donde comprueba si el 
			// paquete recibido es null debido a que se ha implementado 
			// la parte del código que recibe el paquete, por lo que el código 
			// comentado no es necesario y así nos ahorarmos un warning de "dead code".
	...
	private void sendResponseTestMode(DatagramPacket pkt) throws IOException {
		...
		byte[] cadenaArray = pkt.getData();
		String cadena = new String(cadenaArray, 0, pkt.getLength());
		...
		// Primero, vemos si la cadena empieza por ping.
		// Dentro de poder comenzar con ping, tenemos 2 posibilidades.
		if(cadena.startsWith("ping")) {
			// Posibilidad 1, empieza con ping&.
			if(cadena.startsWith("ping&")) {
				String protocolo = cadena.substring(5);
				if(protocolo.equals(NanoFiles.PROTOCOL_ID)) {
					String respuesta = "welcome";
					byte[] respuestaArray = respuesta.getBytes();
					DatagramPacket pcktResponse = new DatagramPacket(respuestaArray, respuestaArray.length, pkt.getAddress(), pkt.getPort());
					socket.send(pcktResponse);
				}
				else {
					String respuesta = "denied";
					byte[] respuestaArray = respuesta.getBytes();
					DatagramPacket pcktResponse = new DatagramPacket(respuestaArray, respuestaArray.length, pkt.getAddress(), pkt.getPort());
					socket.send(pcktResponse);
				}
			} else if (cadena.startsWith("ping")){
				// Posibilidad 2, es un ping a secas.
				String respuesta = "pingok";
				byte[] respuestaArray = respuesta.getBytes();
				DatagramPacket pcktResponse = new DatagramPacket(respuestaArray, respuestaArray.length, pkt.getAddress(), pkt.getPort());
				socket.send(pcktResponse);
			}
	...
	private void sendResponse(DatagramPacket pkt) throws IOException {
		...
		String receivedMessageAsString = new String(pkt.getData(), 0 , pkt.getLength());
		System.out.println("DATA RECEIVED: " + receivedMessageAsString);
		DirMessage receivedMessage = DirMessage.fromString(receivedMessageAsString);
		...
		case DirMessageOps.OPERATION_PING: {
			...
			if(protocolId.equals(NanoFiles.PROTOCOL_ID)) {
				...
				msgToSend = new DirMessage(DirMessageOps.OPERATION_PING_NOK);
			...
			if(resultado)
				System.out.println("Ping... received protocolId: " + protocolId + "\nProtocolId received is compatible - success."); 
			else
				System.out.println("Ping... received protocolId: " + protocolId + "\nProtocolId received is uncompatible.");

			break;
		}
		case DirMessageOps.OPERATION_FILELIST: {
			msgToSend = new DirMessage(DirMessageOps.OPERATION_FILELIST_RESPONSE);
			
			for (FileInfo fi : ficheros.values()) {
				msgToSend.addFileInfo(fi);
			}
			break;
		}
		case DirMessageOps.OPERATION_SERVE: {
			System.out.println("Serve... received");
			for (FileInfo fi : receivedMessage.getFileList()) {
				if (ficheros.containsKey(fi.fileHash)) {
					ficheros.get(fi.fileHash).servidores.add(new InetSocketAddress(clientAddr.getHostString(), receivedMessage.getSourceTCPPort()));
					ficheros.get(fi.fileHash).fileName = fi.fileName;
				} else {
					ficheros.put(fi.fileHash, fi);
					ficheros.get(fi.fileHash).servidores.add(new InetSocketAddress(clientAddr.getHostString(), receivedMessage.getSourceTCPPort()));
				}
		...
		case DirMessageOps.OPERATION_SEARCH: {
			...
			msgToSend = new DirMessage(DirMessageOps.OPERATION_SEARCH_UNSUCCESS);
			for (FileInfo fi : ficheros.values()) {
				if (fi.fileName.contains(receivedMessage.getBusqueda())) {
					msgToSend = new DirMessage(DirMessageOps.OPERATION_SEARCH_SUCCESS);
					msgToSend.setDirList(new LinkedList<InetSocketAddress>(fi.servidores));
				}
			}			
			break;
		}
		...
		case DirMessageOps.OPERATION_QUIT: {
			...
			InetSocketAddress ina = new InetSocketAddress(clientAddr.getHostString(), receivedMessage.getQuitPort());
			LinkedList<FileInfo> ficherosBorrar = new LinkedList<FileInfo>();
			for (FileInfo fi : ficheros.values()) {
				fi.servidores.remove(ina);
				if (fi.servidores.size() == 0) {
					ficherosBorrar.add(fi);
				}
			}
			for (FileInfo fi : ficherosBorrar) {
				ficheros.remove(fi.fileHash);
			}
			msgToSend = new DirMessage(DirMessageOps.OPERATION_QUIT_SUCCESS);
			break;
		...
		if(msgToSend != null) {
			...
			byte dataToClient[] = msgToSendAsString.getBytes();
