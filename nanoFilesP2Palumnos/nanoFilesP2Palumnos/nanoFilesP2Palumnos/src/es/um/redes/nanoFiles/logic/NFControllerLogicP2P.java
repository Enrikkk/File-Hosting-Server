...
public class NFControllerLogicP2P {
	...
	protected boolean startFileServer() {
		...
		if (fileServer != null) {
			...
			try {
				...
				fileServer = null;
				System.err.println("Cannot start the file server: " + ex.getMessage());
			}
	            int listeningPort = fileServer.getPort();
	            if (listeningPort > 0) {
	            	serverRunning = true;
	            	System.out.println("NFServer server running on " + listeningPort);
	            } else { 
	...
	protected boolean downloadFileFromServers(InetSocketAddress[] serverAddressList, String targetFileNameSubstring,
		...
		if(file.exists()) {
			System.err.println("El fichero existe, no se descarga");
			return downloaded;
		}
		
		//Iniciamos los sockets
		LinkedList<NFConnector> servidoresActivos = new LinkedList<NFConnector>();
		for (InetSocketAddress serverAddress : serverAddressList) {
			try {
				servidoresActivos.add(new NFConnector(serverAddress));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		
		//Pasamos el semaforo a las conexiones de los servidores
		NFConnector.NUM_THREADS_WAITING = servidoresActivos.size();
		NFConnector.GLOBAL_OFFSET = 0;
		Semaphore semaf = new Semaphore(servidoresActivos.size());
		try {
			semaf.acquire(servidoresActivos.size());
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		Semaphore parent_signal = new Semaphore(0);
		
		// Ejecutamos la conexión de los servidores
		for (NFConnector server : servidoresActivos) {
			server.parent_signal = parent_signal;
			server.barrier = semaf;
			server.setFileRequest(targetFileNameSubstring);
			server.start();
		}
		
		//Esperamos a que todos terminen el handshake
		try {
			parent_signal.acquire();
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		// 	* FicheroInfo de comprobación
		FileInfo fi = null;
		for(NFConnector server : servidoresActivos) {
			FileInfo cfi = server.getRespuestaFichero();
			if(cfi != null) {
				fi = cfi;
				break;
			}
		...
		if(fi != null) {
			for (InetSocketAddress serverAddress : serverAddressList)  {
				System.out.println("Server at "+ serverAddress + " found matching filename with hash: " + fi.fileHash);
			}
			System.out.println("Downloading file " + targetFileNameSubstring + "...");	
		}
		
		// * Comprobamos el mismo hash
		boolean ambiguo = false;
		if(fi != null) {
			for(int i = 0; i < servidoresActivos.size(); i++) {
				FileInfo cfi = servidoresActivos.get(i).getRespuestaFichero();
				if(cfi != null && !cfi.fileHash.equals(fi.fileHash)) {
					ambiguo = true;
					NFConnector.AMBIGUO = true;
				}				
			}
		}
		...
		if(fi != null && ambiguo == false) {
			// Actualizamos los ficheros
			try {
				fich = new RandomAccessFile(localFileName, "rws");
				NFConnector.FICHERO = fich;
		...
		// Empezamos a descargar los ficheros
		semaf.release(servidoresActivos.size());
		
		// Esperas a que terminen los hilos
		for (NFConnector nf : servidoresActivos) {
			try {
				nf.join();
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
		...
		// Verificamos que el fichero es correcto
		if (ambiguo == false) {
			if (FileDigest.computeFileChecksumString(localFileName).equals(fi.fileHash)) {
				System.out.println("File integrity check passed: FileDigest.computeFileChecksumString returned " + FileDigest.computeFileChecksumString(localFileName));
				System.out.println("Successfully downloaded remote file to " + file.getAbsolutePath());
				downloaded = true;
			} else {
				System.err.println("El hash del fichero no es correcto");
			}
			System.out.println("* Download summary *");
			for (NFConnector nf : servidoresActivos) {
				System.out.println("\t" + nf.getByteRecibido() + " bytes (" + nf.getChunkRecibido() + " chunks) from server at" + nf.getServerAddr() );
			}
		}else {
			System.err.println("Los ficheros son ambiguos");
		}
	...
	protected boolean uploadFileToServer(FileInfo matchingFile, String ipPort, String tcpPort) {
		boolean result = false;

		InetSocketAddress ina = new InetSocketAddress(ipPort, Integer.parseInt(tcpPort));
		try {
			NFConnector nf = new NFConnector(ina);
			result = nf.uploadFile(matchingFile);
			nf.closeConnection();
